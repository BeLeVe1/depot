...
<body>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10">
		<span class="l" style="margin-left:5px;margin-top:1%;">
			<button onclick="open_layer('信息添加','adminfd_outbound_item_propertiesadd','900','450')" href="javascript:;" class="btn btn-success" style="background-color: #337AB7;" type="submit"></i><i class="Hui-iconfont">&#xe645;</i> 添加信息</button>
<!-- 			<button onclick="Excelbutton();" class="btn btn-success" style="background-color: #337AB7;" type="submit"></i><i class="Hui-iconfont">&#xe644;</i> 备份</button> -->
		...
		<div class="text-c" style="margin-top:1%;">
			<input type="text" name="jqcode" id="jqcode" placeholder="项目属性编号" style="width:100px" class="input-text">
			<input type="text" name="bdcode" id="bdcode" placeholder="项目属性名称" style="width:100px" class="input-text">
			...
			<table id="example" class="table table-striped table-bordered">
				<thead>
					<tr>
						...
						<th style="text-align: center;width: 1%">项目属性编号</th>
						<th style="text-align: center;width: 1%">项目属性名称</th>
...
function selectbutton1(){
	...
	$(document).ready(function() {
	     ...
	     $('#example').DataTable( {
	        "ajax":{
	        	  "url": "adminfd_outbound_item_propertieslist1ajax",
	              "type": "post",
		        ...
		        "columns": [        
				 		...
				 		{ "data": "outbound_item_propertiesNum" },
				    	{ "data": "outbound_item_propertiesName" },
		            { "data": function(obj){
		                	return "<span><center><a onclick=\"open_layer('编辑','adminfd_outbound_item_propertiesreset?outbound_item_propertiesNum="+obj.outbound_item_propertiesNum+"','500','350')\">编辑</a>&nbsp;<a onclick=\"deleteobj("+obj.outbound_item_propertiesNum+")\">删除</a></center></span>"
		                	}
		                 }
		        			],
		        "columnDefs": [
...
function Excelbutton(){
	$.ajax({
	    type: "POST",
	    url:'adminfd_outbound_item_propertieslistExcel',
...
function deleteobj(id){
	layer.confirm('确定要执行此删除操作吗?', {icon: 3, title:'提示'}, function(index){
		  $.ajax({
			    cache: true,
			    type: "POST",
			    url:'adminfd_outbound_item_propertiesdeleteajax',
			    data:{id:id},
			    async: false,
