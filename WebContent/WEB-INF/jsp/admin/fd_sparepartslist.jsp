...
<body>
	...
	<div class="cl pd-5 bg-1 bk-gray mt-10">
		<span class="l" style="margin-left:5px;margin-top:1%;">
			<button onclick="open_layer('备品备件信息添加','adminfd_sparepartsadd','900','450')" href="javascript:;" class="btn btn-success" style="background-color: #337AB7;" type="submit"></i><i class="Hui-iconfont">&#xe645;</i> 添加备品备件信息</button>
<!-- 			<button onclick="Excelbutton();" class="btn btn-success" style="background-color: #337AB7;" type="submit"></i><i class="Hui-iconfont">&#xe644;</i> 备份</button> -->
		...
		<div class="text-c" style="margin-top:1%;">
			<input type="text" name="jqcode" id="jqcode" placeholder="备品备件编号" style="width:100px" class="input-text">
			<input type="text" name="bdcode" id="bdcode" placeholder="备品备件名称" style="width:100px" class="input-text">
			...
			<table id="example" class="table table-striped table-bordered">
				<thead>
					<tr>
						...
						<th style="text-align: center;width: 1%">备品备件编号</th>
						<th style="text-align: center;width: 1%">备品备件类型</th>
						
						<th style="text-align: center;width: 1%">备品备件名称</th>
						<th style="text-align: center;width: 1%">备品备件型号</th>
						<th style="text-align: center;width: 1%">备品备件库存</th>
						<th style="text-align: center;width: 1%">备品备件计量单位</th>
						<th style="text-align: center;width: 1%">备品备件单价</th>
...
function selectbutton1(){
	...
	$(document).ready(function() {
	     ...
	     $('#example').DataTable( {
	        "ajax":{
	        	  "url": "adminfd_sparepartslist1ajax",
	              "type": "post",
		        ...
		        "columns": [        
				    	...
				    	{ "data": "sparepartsStocks" },
				    	{ "data": "sparepartsUnit" },
		
			            { "data": "sparepartsPrice" },
		            { "data": function(obj){
		                	return "<span><center><a onclick=\"open_layer('编辑','adminfd_sparepartsreset?sparepartsNum="+obj.sparepartsNum+"','500','350')\">编辑</a>&nbsp;<a onclick=\"deleteobj("+obj.sparepartsNum+")\">删除</a></center></span>"
		                	}
		                 }
		        			],
		        "columnDefs": [
...
function Excelbutton(){
	$.ajax({
	    type: "POST",
	    url:'adminfd_sparepartslistExcel',
...
function deleteobj(id){
	layer.confirm('确定要执行此删除操作吗?', {icon: 3, title:'提示'}, function(index){
		  $.ajax({
			    cache: true,
			    type: "POST",
			    url:'adminfd_sparepartsdeleteajax',
			    data:{id:id},
			    async: false,
